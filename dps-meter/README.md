...
- Below the draggable title, the header contains only the lock on the left and
  the encounter timer on the right. The body contains player/skill rows without a
  mode label or combined damage summary.
- The meter shows the current encounter. Your character's native combat-exit
  event clears it immediately, even if another party member's combat flag stays
  set. The next fight starts at `0:00` with fresh totals, including when it begins
  within eight seconds of the previous one.
- The timer and DPS advance on damage events, so they stop changing after the
  final hit. Eight seconds without party damage remains a fallback if no exit
  event arrives. A flag or resting heal alone cannot start another encounter.
...
original game method run. A postfix on `ent.Hero.onLeaveCombat` closes the current
encounter when the local character exits combat. Party information comes from
