...
Minion hits are credited to their summoner and grouped under the skill that
created that specific minion. The collector follows the replicated
`summonOwner` and `summonSourceSkill` links and the game's source-skill resolver,
including nested summons and status/child skills. Multiple minions from the same
ability share one attack row for their player; other players' summons stay in
their own totals. Each damage event is counted once in the live meter, rift
recap and reports. If a source-skill link is unavailable, the original attack
skill is retained rather than guessed from the minion's name or equipped weapon.
Cast counts remain estimates based on gaps between hits, not exact summon counts.
