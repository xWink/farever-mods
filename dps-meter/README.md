...
- The timer starts on your character's native combat entry and runs continuously
  while you remain in combat, including pauses in damage. Combat exit freezes
...
original game method run. Postfixes on `ent.Hero.onEnterCombat` and
`ent.Hero.onLeaveCombat` start and freeze the local encounter timer. Party information comes from
