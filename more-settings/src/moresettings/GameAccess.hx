...
class GameAccess {
    ...
    static var types:Map<String, Bool> = [];
    public static function isA(object:Dynamic, base:String):Bool {
        if (object == null) return false;
        var type = hl.Type.getDynamic(object);
        var key = type.getTypeName() + ":" + base;
        if (types.exists(key)) return types[key];
        var found = false;
        while (type != null && type.kind == HObj) {
            if (type.getTypeName() == base) { found = true; break; }
            type = type.getSuper();
        }
        types[key] = found;
        return found;
